...
angular.module('dliskApp').controller('accountController', ['$state','$scope', '$rootScope', '$http', "userService", "$interval", "$timeout", "sendTransactionModal", "secondPassphraseModal", "delegateService", 'viewFactory', 'transactionInfo', 'userInfo', '$filter', 'gettextCatalog', function ($state, $rootScope, $scope, $http, userService, $interval, $timeout, sendTransactionModal, secondPassphraseModal, delegateService, viewFactory, transactionInfo, userInfo, $filter, gettextCatalog) {
    ...
    $scope.getCandles = function () {
        $http.get("https://explorer.dlisk.com/api/candles/getCandles").then(function (response) {
            $scope.graphs.dliskPrice.data = [
